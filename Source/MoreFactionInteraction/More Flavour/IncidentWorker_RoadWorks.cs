...
namespace MoreFactionInteraction
{
    class IncidentWorker_RoadWorks : IncidentWorker
    {
        ...
        protected override bool TryExecuteWorker(IncidentParms parms)
        {
            ...
            var destination = Rand.Chance(directConnectionChance) ? map.Tile : AllyOfNearbySettlement(Settlement)?.Tile ?? map.Tile;
            ...
            using (path = Find.WorldPathFinder.FindPath(destination, Settlement.Tile, null))
            {
                ...
                for (var i = 1; i < path.NodesReversed.Count - 1; i++)
                {
                    ...
                    var roadConstruction = (WorldObject_RoadConstruction)WorldObjectMaker.MakeWorldObject(MFI_DefOf.MFI_RoadUnderConstruction);
