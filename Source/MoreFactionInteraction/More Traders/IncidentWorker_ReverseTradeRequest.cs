...
namespace MoreFactionInteraction
{
    public class IncidentWorker_ReverseTradeRequest : IncidentWorker
    {
        ...
        protected override bool TryExecuteWorker(IncidentParms parms)
        {
            ...
            var letterToSend = DetermineLetterToSend(thingCategoryDef: thingCategoryDef);
            var feeRequest = Math.Max(val1: Rand.Range(min: 150, max: 300), val2: (int)parms.points);
            var categorylabel = (thingCategoryDef == ThingCategoryDefOf.PlantFoodRaw) ? thingCategoryDef.label + " items" : thingCategoryDef.label;
            ...
            var traveltime = CalcuteTravelTimeForTrader(originTile: settlement.Tile, map);
        ...
        {
            var rand = Rand.RangeInclusive(min: 0, max: 100);
