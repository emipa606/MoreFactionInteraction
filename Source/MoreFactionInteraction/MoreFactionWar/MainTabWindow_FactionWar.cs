...
#pragma warning restore IDE0044 // Add readonly modifier
        ...
        public static void DrawFactionWarBar(Rect fillRect)
        {
            ...
            var leftFactionLabel = new Rect(x: 0f, y: 0f, width: position.width / 2f, height: TitleHeight);
            var leftBox = new Rect(x: 0f, y: leftFactionLabel.yMax, width: leftFactionLabel.width, height: InfoHeight);
            var rightFactionLabel = new Rect(x: position.width / 2f, y: 0f, width: position.width / 2f, height: TitleHeight);
            var rightBox = new Rect(x: position.width / 2f, y: leftFactionLabel.yMax, width: leftFactionLabel.width, height: InfoHeight);

            //big central box
            var centreBoxForBigFactionwar = new Rect(0f, leftFactionLabel.yMax, width: position.width, height: TitleHeight);
            ...
            //"score card" bar
            var leftFactionOneScoreBox = new Rect(0f, yPositionBar, position.width * Find.World.GetComponent<WorldComponent_MFI_FactionWar>().ScoreForFaction(factionOne), barHeight);
            GUI.DrawTexture(leftFactionOneScoreBox, FactionOneColorTexture(factionOne));
            var rightFactionTwoScoreBox = new Rect(position.width * Find.World.GetComponent<WorldComponent_MFI_FactionWar>().ScoreForFaction(factionOne), yPositionBar, position.width * Find.World.GetComponent<WorldComponent_MFI_FactionWar>().ScoreForFaction(factionInstigator), barHeight);
