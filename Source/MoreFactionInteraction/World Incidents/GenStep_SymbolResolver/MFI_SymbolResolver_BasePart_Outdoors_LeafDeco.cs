...
namespace MoreFactionInteraction.World_Incidents.GenStep_SymbolResolver
{
    ...
    class MFI_SymbolResolver_BasePart_Outdoors_LeafDecorated_RandomInnerRect : SymbolResolver
    {
        ...
        public override void Resolve(ResolveParams rp)
        {
            var           num           = Rand.RangeInclusive(min: MinLength, max: rp.rect.Width  - 1);
            var           num2          = Rand.RangeInclusive(min: MinLength, max: rp.rect.Height - 1);
            var           num3          = Rand.RangeInclusive(min: 0, max: rp.rect.Width  - num);
            var           num4          = Rand.RangeInclusive(min: 0, max: rp.rect.Height - num2);
            ResolveParams resolveParams = rp;
