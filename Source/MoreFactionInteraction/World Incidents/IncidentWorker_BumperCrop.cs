...
namespace MoreFactionInteraction.World_Incidents
{
    public class IncidentWorker_BumperCrop : IncidentWorker
    {
        ...
        {
            var num = RandomOfferDuration(tileIdFrom: map.Tile, tileIdTo: target.parent.Tile);
            if (num < 1)
            {
                return false;
            }
        ...
        private static int RandomOfferDuration(int tileIdFrom, int tileIdTo)
        {
            ...
            var travelTimeByCaravan = CaravanArrivalTimeEstimator.EstimatedTicksToArrive(@from: tileIdFrom, to: tileIdTo, caravan: null);
            var daysWorthOfTravel = (float)travelTimeByCaravan / GenDate.TicksPerDay;
            var b = Mathf.CeilToInt(f: Mathf.Max(a: daysWorthOfTravel + 1f, b: daysWorthOfTravel * 1.1f));
